...
class ChatAgent(ShieldRunnerMixin):
    ...
    async def resume_turn(self, request: AgentTurnResumeRequest) -> AsyncGenerator:
        with tracing.span("resume_turn") as span:
            ...
            last_turn_start_time = datetime.now().astimezone()
            if len(turns) > 0:
    ...
    async def run_multiple_shields_wrapper(
        ...
        with tracing.span("run_shields") as span:
            ...
            shield_call_start_time = datetime.now().astimezone()
            try:
                yield AgentTurnResponseStreamChunk(
                    event=AgentTurnResponseEvent(
    ...
    async def _run(
        ...
        while True:
            step_id = str(uuid.uuid4())
            inference_start_time = datetime.now().astimezone()
            yield AgentTurnResponseStreamChunk(
                event=AgentTurnResponseEvent(
                    payload=AgentTurnResponseStepStartPayload(
            ...
            if len(message.tool_calls) == 0:
                ...
                with tracing.span(
                    ...
                    tool_execution_start_time = datetime.now().astimezone()
                    tool_call = message.tool_calls[0]
